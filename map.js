...
const MARKER_ICON_URL = 'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-';
const MARKER_SHADOW_URL = 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/0.7.7/images/marker-shadow.png';
const EVENTS_API_URL = 'https://snowy-wood-82b6.dave-sowerby.workers.dev/events';
...
function parseName(nameevent) {
	return (typeof(nameevent) !== 'undefined') ? nameevent.properties.EventShortName : undefined;
}

function parseLongName(nameevent) {
	return (typeof(nameevent) !== 'undefined') ? nameevent.properties.EventLongName : undefined;
}

function parseEventId(eventidevent) {
	return (typeof(eventidevent) !== 'undefined') ? eventidevent.properties.eventname : undefined;
}

function parseLongitude(longevent) {
	return (typeof(longevent) !== 'undefined') ? parseFloat(longevent.geometry.coordinates[0]) : undefined;
}

function parseLatitude(latevent) {
	return (typeof(latevent) !== 'undefined') ? parseFloat(latevent.geometry.coordinates[1]) : undefined;
}

function parseCountrycode(countrycodeevent) {
	return (typeof(countrycodeevent) !== 'undefined') ? countrycodeevent.properties.countrycode : undefined;
}
...
function initMap() {
	...
	mymap.on('contextmenu', function (eventData) { 
		window.location.hash = '#closest-5-' + eventData.latlng.lat + ',' + eventData.latlng.lng;
	});
}
...
function showLoading() {
	$('#mapid').css('opacity', '0.5').css('pointer-events', 'none');
}

function hideLoading() {
	$('#mapid').css('opacity', '1').css('pointer-events', 'auto');
}
...
function displayEvents(filterFunctions) {
	...
	var eventList = $events.features;
	console.info('initial size: ' + eventList.length);
	console.info('processing with ' + filterFunctions.length + ' filters');

	for (var i = 0; i < filterFunctions.length; i++) {
		var filterFunction = filterFunctions[i];
		eventList = filterFunction(eventList);
		console.info('after filter ' + i + ' there are ' + eventList.length + ' events');
	}

	console.info('after all filters there are ' + eventList.length + ' events');
...
function getMarkerIcon(colour) {
	if (!markerIconCache[colour]) {
		markerIconCache[colour] = L.icon({
			iconUrl: MARKER_ICON_URL + colour + '.png',
...
function filterEvents(eventList, eventFilter) {
...
function createSeriesIdFilter(seriesid) {
	return filterEvents(events, function(event) {
		return parseSeriesId(event) == seriesid;
	});
}

function createNotFilter(innerFilter) {
	var filterFunction = getFilter(innerFilter);
	return function(events) {
		var includedEvents = filterFunction(events);
		return _.differenceWith(events, includedEvents, function(event1, event2) { 
			return parseEventId(event1) == parseEventId(event2); 
		});
	};
}

function createAndFilter(andFilterStr) {
	var andFilters = [];
	andFilterStr.split('&&').filter(function(e){return e}).forEach(function(andFilterPart) {
...
function createOrFilter(orFilterStr) {
	var orFilters = [];
	orFilterStr.split('||').filter(function(e){return e}).forEach(function(orFilterPart) {
...
function createStartsWithFilter(prefix) {
	return filterEvents(events, function(event) {
		var name = parseName(event);
		return name.toLowerCase().startsWith(prefix.toLowerCase()) || 
		       name.toLowerCase().startsWith('parkrun ' + prefix.toLowerCase());
	});
}

function createContainsFilter(needle) {
	return filterEvents(events, function(event) {
		var name = parseName(event);
...
function createMatchesFilter(regexStr) {
...
function createCountryFilter(country) {
	var countryCodeMap = {
		'austria': 4, 'at': 4,
		'australia': 3, 'au': 3,
		'canada': 14, 'ca': 14,
		...
		'lithuania': 54, 'lt': 54,
		'malaysia': 57, 'my': 57,
		'netherlands': 64, 'nl': 64,
		'new-zealand': 65, 'new zealand': 65, 'nz': 65,
		'norway': 67, 'no': 67,
		'poland': 74, 'pl': 74,
		'russia': 79, 'ru': 79,
		'singapore': 82, 'sg': 82,
		'south africa': 85, 'south-africa': 85, 'za': 85,
		'sweden': 88, 'se': 88,
		'united kingdom': 97, 'united-kingdom': 97, 'uk': 97,
		'united states': 98, 'united-states': 98, 'usa': 98, 'us': 98
	};
	var countrycode = countryCodeMap[country.toLowerCase()];
...
function createAthleteFilter(athleteId) {
	...
	if (typeof(athleteData[athleteId]) === 'undefined') {
		...
		return function(events) { return []; };
	}
	
	return filterEvents(events, function(event) {
		var eventId = parseEventId(event);
		return athleteData[athleteId].includes(eventId);
	});
}

function createWithinFilter(withinStr) {
	withinFilter = true;
	var distance, withinLatitude, withinLongitude;
	
	if (isNaN(withinStr)) {
		var indexOf = withinStr.indexOf('-');
		distance = withinStr.substring(0, indexOf);
		var withinLongLat = withinStr.substring(indexOf+1).split(',');
		withinLatitude = withinLongLat[0];
		withinLongitude = withinLongLat[1];
	} else {
		distance = parseInt(withinStr);
...
function createCompassFilter() {
	return function(events) {
		var northern = {}, southern = {}, eastern = {}, western = {};
		...
		for (var e = 0; e<events.length; e++) {
			...
			if (isNaN(northern.latitude) || northern.latitude < latitude) {
				northern = { latitude: latitude, longitude: longitude, id: parseEventId(event), event: event };
			}
			if (isNaN(eastern.longitude) || eastern.longitude < longitude) {
				eastern = { latitude: latitude, longitude: longitude, id: parseEventId(event), event: event };
			}
			if (isNaN(southern.latitude) || southern.latitude > latitude) {
				southern = { latitude: latitude, longitude: longitude, id: parseEventId(event), event: event };
			}
			if (isNaN(western.longitude) || western.longitude > longitude) {
				western = { latitude: latitude, longitude: longitude, id: parseEventId(event), event: event };
			}
		}

		return _.uniq([northern.event, eastern.event, southern.event, western.event]);
	}
}

function createClosestFilter(closestStr) {
	closestFilter = true;
	var closest = closestStr;
	var closestLatitude, closestLongitude;
...
function createPinFilter(pinStr) {
	var pin = pinStr;
	var indexOf = pin.indexOf('-');
	var pinColour = pin.substring(0, indexOf);
	var pinLongLat = pin.substring(indexOf+1).split(',');
	var pinLatitude = pinLongLat[0];
	var pinLongitude = pinLongLat[1];
...
function getFilter(filter) {
	if (filter.startsWith(FILTER_TYPES.SERIES_ID)) {
		return createSeriesIdFilter(filter.substring(9));
	} else if (filter.startsWith(FILTER_TYPES.NOT)) {
		return createNotFilter(filter.substring(4));
	} else if (filter.startsWith(FILTER_TYPES.AND)) {
		return createAndFilter(filter.substring(4));
	} else if (filter.startsWith(FILTER_TYPES.OR)) {
		return createOrFilter(filter.substring(3));
	} else if (filter.startsWith(FILTER_TYPES.STARTS_WITH)) {
		return createStartsWithFilter(filter.substring(11));
	} else if (filter.startsWith(FILTER_TYPES.CONTAINS)) {
		return createContainsFilter(filter.substring(9));
	} else if (filter.startsWith(FILTER_TYPES.MATCHES)) {
		return createMatchesFilter(filter.substring(8));
	} else if (filter.startsWith(FILTER_TYPES.COUNTRY)) {
		return createCountryFilter(filter.substring(8));
	} else if (filter.startsWith(FILTER_TYPES.ATHLETE)) {
		return createAthleteFilter(filter.substring(8));
	} else if (filter.startsWith(FILTER_TYPES.WITHIN)) {
		return createWithinFilter(filter.substring(7));
	} else if (filter == FILTER_TYPES.COMPASS) {
		return createCompassFilter();
	} else if (filter.startsWith(FILTER_TYPES.CLOSEST)) {
		return createClosestFilter(filter.substring(8));
	} else if (filter == FILTER_TYPES.RANDOM) {
		return createRandomFilter();
	} else if (filter === FILTER_TYPES.NONE) {
		return filterEvents(events, function(event) {
			return [];
		});
	...
	} else if (filter.startsWith(FILTER_TYPES.PIN)) {
		return createPinFilter(filter.substring(4));
	}
}
...
function load() {
	$(document).ready(function() {
		...
		hash.split('#').filter(function(e){return e}).forEach(function(hashPart) {
			console.info('filter ' + h + ' ' + hashPart);
			var filter = getFilter(hashPart);
			if (filter !== undefined) {
				filters.push(filter);
			}
			h++;
...
function init() {
	...
	.fail(function(jqXHR, textStatus, errorThrown) {
		...
		noty({ text: 'Failed to load events. Please refresh the page.', type: 'error' });
	});
